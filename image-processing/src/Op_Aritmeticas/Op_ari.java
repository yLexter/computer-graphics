...
public class Op_ari {
    ...
    // Método para somar duas imagens
    public BufferedImage soma(BufferedImage img1, BufferedImage img2) {
        return processar(img1, img2, (p1, p2) -> Math.min(p1 + p2, 255));
    }

    // Método para subtrair duas imagens
    public BufferedImage subtracao(BufferedImage img1, BufferedImage img2) {
        return processar(img1, img2, (p1, p2) -> Math.max(p1 - p2, 0));
    }

    // Método para multiplicar duas imagens
    public BufferedImage multiplicacao(BufferedImage img1, BufferedImage img2) {
        return processar(img1, img2, (p1, p2) -> Math.min(p1 * p2 / 255, 255));
    }

    // Método para dividir duas imagens
    public BufferedImage divisao(BufferedImage img1, BufferedImage img2) {
        return processar(img1, img2, (p1, p2) -> p2 == 0 ? 255 : Math.min(p1 * 255 / p2, 255));
    }
